...
func FetchCurrentWayNodes(ids []int64) ([]gormModel.CurrentWayNodes, error) {
	return fetchCurrentWayNodes(dbClient.DatabaseClients.GetRandomConnection(), ids)
}
func FetchCurrentWayNodesByWaysIds(ids []int64) ([]gormModel.CurrentWayNodes, error) {
	return fetchCurrentWatNodesByWayIds(dbClient.DatabaseClients.GetMasterConnection(), ids)
}

func fetchCurrentWayNodes(client *gorm.DB, nodeIds []int64) ([]gormModel.CurrentWayNodes, error) {
...
func fetchCurrentWatNodesByWayIds(client *gorm.DB, ids []int64) ([]gormModel.CurrentWayNodes, error) {
	//var currentWayNodes []gormModel.CurrentWayNodes
	//query := client.Model(&gormModel.CurrentWayNodes{})
	//query = query.
	//	Joins("JOIN current_nodes ON current_nodes.id = current_way_nodes.node_id").
	//	Joins("LEFT JOIN node_tags ON current_nodes.id = node_tags.node_id").
	//	Joins("LEFT JOIN changesets ON current_nodes.changeset_id = changesets.id").
	//	Joins("LEFT JOIN users ON changesets.user_id = users.id").
	//	Where("current_way_nodes.way_id IN ?", ids)
	//
	//err := query.Find(&currentWayNodes).Error
	//if err != nil {
	//	return currentWayNodes, err
	//}
	//return currentWayNodes, nil
